...
```cmd
:: Build distribution zips (run on Ajay's machine)
build_dist.bat
:: Select [1], [2], [3], or [A] to build all
```

The zips are created in the `dist/` directory. Share the appropriate zip with each team member.

---

## Prerequisites

- Python 3.7 or higher (not needed for Windows Full zip)
...
**Windows (from zip):**
```cmd
:: Extract the zip, then run install.bat
:: It auto-detects embedded Python (if present) or system Python
install.bat
```
...
The installer handles: Python detection, dependency install (skipped if pre-bundled), setup wizard, overhead config, scheduled tasks (Task Scheduler / cron), wrapper script generation, and tray app setup.
...
run_daily.bat                # Task Scheduler wrapper (regenerated by install.bat)
